...
## 30. 可解释推理轨迹（2026-09-06）

- Supervisor 在模型调用前后记录高层推理事件：模型正在分析下一步、选择下一项工具、确定回复方式，以及 provider 失败后的安全策略；事件不包含原始隐藏思维 token、系统提示词或私有推理文本。
- 联网搜索活动补充返回条数和可追溯来源数量；前端按“思考 / 搜索 / 工具 / 结果 / 保存”展示阶段标签，并使用“推理过程”折叠面板承载完整轨迹。
- 旅行规划在同步 Supervisor 和 provider 编排前刷新阶段事件，等待外部数据时用户能看到当前工作阶段；`done.activities` 仍是历史与最终状态的权威结构。
- 验证：推理轨迹单元测试、搜索事件测试、前端 typecheck/build 和 Playwright 桌面截图回归通过，控制台无错误。

## 31. 推理轨迹归档与失败状态修正（2026-09-07）

- 修正实时 activity 被消费后无法写入历史的问题：新增独立归档队列，实时 SSE 与 assistant 历史 metadata 各自读取，刷新会话后仍保留完整推理轨迹。
- 模型重复工具调用会立即停止当前 Supervisor 循环，并基于已验证结果采用安全默认动作，避免同一推理步骤无限重复。
- provider/请求失败时，仍在进行中的轨迹步骤显示“未完成”，不再误显示为“已完成”。
- 最终验证：`python -m pytest -q` 为 **163 passed**、1 个既有 Starlette 依赖弃用警告；`compileall`、前端 `npm run typecheck`、`npm run build`、`git diff --check` 和真实 SSE/Playwright 回归均通过。

## 32. Codex 风格公开工作进展（2026-09-07）

- Supervisor 提示词要求在工具调用前输出不超过 120 字的 `<public_progress>`；后端只提取普通 assistant 文本（兼容单行短摘要）并做长度、敏感词和结构过滤，记录 `origin=model`。模型未返回摘要时，代码根据真实工具调用生成安全兜底句，记录为 `origin=system`，不增加额外模型请求。
- 前端活动面板改为公开工作进展样式：模型摘要以自然语言段落显示，代码/Provider 状态以简洁操作行显示；SSE、历史归档、停止和重试链路继续复用原有 `activity` 契约。
- 工作进展改为 SSE 事件驱动：后端每产生一个真实阶段事件，前端立即追加对应一条，顺序跟随 Supervisor/Provider 的执行顺序，不使用定时器伪造进度；展开区域限制为独立滚动区，避免长列表把聊天正文整体挤出视口；公开摘要和状态文字同步放大。
