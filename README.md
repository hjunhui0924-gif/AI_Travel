...
用户明确指定多个地点时，后端按有界预算查询有向路线矩阵，并优先保证用户给出的相邻顺序。路线缓存仅复用成功的结构化 provider 结果；预算不足或路线失败会在计划诊断中标记缺边，不会伪造路线。`route_segments`、地图 polyline 和路线计划来自同一批最终选中的路线边；未知费用不会被当作 0 元，也不能据此宣称最低价。
